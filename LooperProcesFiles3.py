...
def process_files3(matrix_bytes, sales_bytes) -> dict:
    try:
...
def generate_report2(matrix_data, sales_data):
    ...
    for index, sale in enumerate(sales_data, start=1):
        ...
        for item in matching_items:
            ...
            peligrosidad = item.get('Peligrosidad', '').strip().lower()
            if 'no domiciliario' in categoria:
                add_to_sumadores2(sumadores['no domiciliario'], material, peso_total, peligrosidad)
            elif 'domiciliario' in categoria:
                add_to_sumadores2(sumadores['domiciliario'], material, peso_total, peligrosidad)
            else:
...
def initialize_sumadores2():
    ...
    def create_nested_sumadores(materials):
        return {material: {"peligroso": 0.0, "no peligroso": 0.0} for material in materials}
    ...
    return {
        "domiciliario": create_nested_sumadores(materials_domiciliario),
        "no domiciliario": create_nested_sumadores(materials_no_domiciliario),
    }
...
def add_to_sumadores2(sumadores, material, peso, peligrosidad):
    if material in sumadores:
        key = "peligroso" if peligrosidad.lower().strip() == "peligroso" else "no peligroso"
        sumadores[material][key] += peso



def format_report2(sumadores):
    """Convierte sumadores en lista de diccionarios con columnas separadas por peligrosidad."""
    report = []
